...
class Listener(myo.DeviceListener):
    ...
    def on_gyroscope_data(self, myo, timestamp, gyroscope):
        ...
        if self.horizontal:
            if gyroscope[0] > 10:
                ...
                self.listOfPositions.append(self.position)
    ...
    def on_accelerometor_data(self, myo, timestamp, acceleration): 
        ...
        if newValue[2] > 1: pass #print acceleration
        # If acceleration is ever significant, the above conditional happens.
    ...
    def on_pose(self, myo, timestamp, pose):
        ...
        if pose == pose_t.double_tap:
            ...
            for index in xrange(len(self.listOfPixels)):
                self.canvas.coords(self.listOfPixels[index], self.listOfPositions[index][0] + 20, self.listOfPositions[index][1],
                                                            self.listOfPositions[index][0] + self.r + 20, self.listOfPositions[index][1] + self.r)
                self.listOfPositions[index] = (self.listOfPositions[index][0] + 20, self.listOfPositions[index][1])
